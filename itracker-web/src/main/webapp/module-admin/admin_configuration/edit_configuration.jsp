...
<html:form action="/editconfiguration">
  ...
  <table border="0" cellspacing="0"  cellpadding="1"  width="100%" class="shadeList">
        ...
        <tr>
          <td colspan="4">
             <span class="editColumnTitle">
              <c:choose>
                  <c:when test="${ action=='createstatus' }"><it:message key="itracker.web.attr.status"/></c:when>
                  <c:otherwise><it:message key="${configurationForm.typeKey}"/></c:otherwise>
              </c:choose>

              <it:message key="itracker.web.attr.value"/>:
              <c:choose>
                  <c:when test="${ action=='createstatus' }"><html:text property="value" styleClass="editColumnText"/></c:when>
    ...
    <tr class="listRowShaded">
      ...
      <td>
        <html:text property="${baseLocaleKey}" styleId="BASE" styleClass="editColumnText"/></td>
