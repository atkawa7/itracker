...
<html:form action="/searchissues">
   ...
   <table border="0" cellspacing="0" cellspacing="1" align="left" width="800px">
      ...
      <tr>
         ...
         <td valign="top" class="editColumnText"><%= query.getProjectName() %>
         </td>
         <% } else { %>
      ...
      <tr>
         <td colspan="99"><html:img module="/" page="/themes/defaulttheme/images/blank.gif" width="1" height="12"/></td>
      </tr>
      <tr>
         ...
         <td valign="top" class="editColumnText">
            ...
            <html:select property="statuses" styleClass="editColumnText" size="5" multiple="true">
               ...
               <html:option value="<%= statuses.get(i).getValue() %>"
                            styleClass="editColumnText"><%= IssueUtilities.getStatusName(statuses.get(i).getValue(), LoginUtilities.getCurrentLocale(request)) %>
               </html:option>
               <% } %>
            </html:select>
         </td>
      ...
      <tr>
         ...
         <td valign="top" class="editColumnText">
            <html:select property="resolution" styleClass="editColumnText">
               ...
               %>
               <html:option value="<%= possResolutions.get(i).getValue() %>"><%= possResolutions.get(i).getName() %>
               </html:option>
               <% } %>
            </html:select>
         </td>
         <% } else { %>
         <td valign="top" class="editColumnText"><html:text property="resolution" size="20"
                                                            styleClass="editColumnText"/></td>
         <% } %>
         <td valign="top" class="editColumnTitle"><it:message key="itracker.web.attr.phrase"/>:</td>
         <td valign="top" class="editColumnText"><html:text property="textphrase" size="30"
                                                            styleClass="editColumnText"/></td>
      </tr>
      <tr>
         <td colspan="99"><html:img module="/" page="/themes/defaulttheme/images/blank.gif" width="1" height="18"/></td>
      </tr>
      <tr>
         <td colspan="99" align="left"><html:submit styleClass="button" altKey="itracker.web.button.search.alt"
                                                    titleKey="itracker.web.button.search.alt"><it:message
                 key="itracker.web.button.search"/></html:submit></td>
      </tr>
   </table>
</html:form>
</td></tr>
   ...
   <td>
      ...
      <table border="0" cellspacing="0" cellspacing="1" width="100%">
         <tr>
            ...
            <td class="listRowText" colspan="2" align="right"><it:message key="itracker.web.generic.totalissues"
                                                                          arg0="<%= Integer.toString(issues.size()) %>"/></td>
         </tr>
         ...
         <tr align="right" class="<%= (i % 2 == 1 ? "listRowShaded" : "listRowUnshaded" ) %>">
               ...
               <% if (!IssueUtilities.hasIssueNotification(issues.get(i), currUserId)) { %>
                    <span class="HTTP_POST">
                        <it:formatImageAction forward="watchissue" paramName="id"
                                              paramValue="<%= issues.get(i).getId() %>" caller="index"
            ...
            <td></td>
            <td><%= issues.get(i).getId() %>
            </td>
            <td><%= issues.get(i).getProject().getName() %>
            </td>
            <td><%= IssueUtilities.getStatusName(issues.get(i).getStatus(), LoginUtilities.getCurrentLocale(request)) %>
            </td>
            <td><%= IssueUtilities.getSeverityName(issues.get(i).getSeverity(), LoginUtilities.getCurrentLocale(request)) %>
            </td>
            <td><%= (issues.get(i).getComponents().size() == 0 ? ITrackerResources.getString("itracker.web.generic.unknown", LoginUtilities.getCurrentLocale(request)) : issues.get(i).getComponents().get(0).getName() + (issues.get(i).getComponents().size() > 1 ? " (+)" : "")) %>
            </td>
            <td><it:formatDescription><%= issues.get(i).getDescription() %>
         ...
         <tr class="listRowUnshaded" align="left">
            <td colspan="10" align="left"><it:message key="itracker.web.error.noissues"/></td>
         </tr>

         <% } else { %>
         <html:form action="/displayreport">
            <tr>
               <td colspan="99"><html:img module="/" page="/themes/defaulttheme/images/blank.gif" width="1"
                                          height="10"/></td>
            </tr>
            <tr class="listRowUnshaded" align="left" style="vertical-align: top;">
               <td colspan="10" align="left" style="vertical-align: top;">
                  <html:select property="reportId" styleClass="listRowUnshaded" style="vertical-align: top;">
                     ...
                     %>
                     <html:option value="<%= reports.get(i).getId().toString() %>"
                                  key="<%= reports.get(i).getNameKey() %>"/>
                     <% } else { %>
                     <html:option value="<%= reports.get(i).getId().toString() %>"><%= reports.get(i).getName() %>
                     </html:option>
                     <%
                           }
                        }
