...
<%--<html:link forward="/selfregistration"--%>
                             ...
                             <%--titleKey="itracker.web.header.menu.selfreg.alt"> Test 1</html:link>--%>
<html:link forward="selfregistration"
                             styleClass="headerLinks"
                             titleKey="itracker.web.header.menu.selfreg.alt"> Test 2</html:link>
<html:link forward="test"
                             styleClass="headerLinks"
                             titleKey="itracker.web.header.menu.selfreg.alt"> Test 2</html:link>
<%--<html:link forward="selfregistration.do"--%>
                             <%--styleClass="headerLinks"--%>
										...
										<c:choose>
											<c:when test="${unassignedIssue.unassigned}">
												...
												<option value="-1"><c:out
													value="${itracker_web_generic_unassigned}" /></option>
											</c:when>
											<c:otherwise>
		...
		<c:choose>
			<c:when test="${showAll || (i.count <=userPrefs.numItemsOnIndex)}">
				...
				<tr id="createdIssue.${i.count}" class="${listRowClass}">
					...
					<td></td>
					<td style="text-align: right; white-space: nowrap"><it:formatDate
						date="${createdIssue.issue.lastModifiedDate}" /></td>
		...
		<tr style="text-align: left;" class="listRowUnshaded">
			<td colspan="15" style="text-align: left;"><c:choose>
				<c:when test="${!allSections}">
					<html:link action="/portalhome?allSections=true">
						<it:message key="itracker.web.index.viewhidden" />
					</html:link>
				</c:when>
				<c:otherwise>
					<html:link action="/portalhome?allSections=false">
						<it:message key="itracker.web.index.hidehidden" />
					</html:link>
				</c:otherwise>
			</c:choose></td>
		</tr>
		<tr>
			<td><html:img page="/themes/defaulttheme/images/blank.gif"
