			...
			<table border="0" cellspacing="0" cellspacing="1" width="800">
				<tr>
					<td class="editColumnTitle">
					   <it:message	key="itracker.web.attr.name" />:
					</td>
					<td>
					   <html:text property="name" styleClass="editColumnText" />
					</td>
					<td valign="top" class="editColumnTitle">
					...
					<td valign="top" class="editColumnText">
					   <html:select	property="status" styleClass="editColumnText">
						<c:forEach var="status" items="${statuses}">
									<html:option styleClass="editColumnText" value="${status.value}">
									   ${status.name}
    	        </html:option>
						</c:forEach>
					   </html:select>
				...
				<tr>
				  <td class="editColumnTitle">
					...
					<td>
					  <html:text property="description"	styleClass="editColumnText" />
					</td>
					<td class="editColumnTitle">
					  <it:message	key="itracker.web.attr.created" />:
					</td>
						...
						<c:otherwise>
							<c:choose>
								<c:when test="${allowPermissionUpdateOption}">
									<td valign="top" class="editColumnTitle">
									  <it:message	key="itracker.web.admin.editproject.addusers" />:
									</td>
									<td valign="top" class="editColumnText" nowrap>
									  ...
									  <html:select property="permissions" size="5" multiple="true" styleClass="editColumnText">
										<c:forEach var="permission" items="${permissions}">
												  <html:option value="${permission.value}">${permission.name}</html:option>
										</c:forEach>
									  </html:select>
									</td>
								</c:when>
				...
				<tr class="editColumnTitle">
					<td colspan="4">
					  <it:message	key="itracker.web.admin.editproject.options" />:
					</td>
				</tr>
				<tr class="listHeading">
					<td colspan="4">
					  <html:img module="/" page="/themes/defaulttheme/images/blank.gif" height="2" width="1" />
					</td>
				</tr>
				<tr>
					<td colspan="4">
					  <html:img module="/" page="/themes/defaulttheme/images/blank.gif" height="3" width="1" />
					</td>
				</tr>
				<tr>
					<td width="25"></td>
				...
				<tr>
					<td colspan="2" class="editColumnText">
						...
						<it:message	key="itracker.web.admin.editproject.options.html" />
					</td>

					<td colspan="2" class="editColumnText">
						<html:multibox property="options"	value="${optionLiteralHistoryHtml}" />
						<it:message	key="itracker.web.admin.editproject.options.htmlliteral" />
					</td>
				</tr>
				<tr>
					<td colspan="2" class="editColumnText">
						<html:multibox property="options"	value="${optionPredefinedResolutions}" /> 
						<it:message	key="itracker.web.admin.editproject.options.resolution" />
					</td>

					<td colspan="2" class="editColumnText">
						<html:multibox property="options"	value="${optionNoAttachments}" />
						<it:message	key="itracker.web.admin.editproject.options.noattach" />
					</td>
				</tr>
				<tr>
					<td colspan="2" class="editColumnText">
						<html:multibox property="options"	value="${optionAllowAssignToClose}" />
						<it:message	key="itracker.web.admin.editproject.options.closed" />
					</td>
					<c:if test="${allowSelfRegister}">

						<td colspan="2" class="editColumnText">
							<html:multibox property="options" value="${optionAllowSelfRegisteredViewAll}" />
							<it:message	key="itracker.web.admin.editproject.options.srview" />
						</td>
					</c:if>
				</tr>
				<c:if test="${allowSelfRegister}">
					<tr>
						<td colspan="2" class="editColumnText">
							...
							<it:message key="itracker.web.admin.editproject.options.srcreate" />
						</td>
					</tr>
				</c:if>
		...
		<td>
		  <c:if test="${isUpdate}">
			<table border="0" cellspacing="0" cellspacing="1" width="100%">
				...
				<tr>
				   <td class="editColumnTitle" colspan="5">
