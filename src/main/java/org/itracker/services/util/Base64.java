 ...
 * Base64 Original Author: Robert Harder
 * Base64 Original Author: rob@iharder.net
 */
...
/**
 * What's this stuff used for? please comment
 * 
 * @author ready
 * 
 */
public class Base64 {
		...
		if (bytes.length >= 2) {
			...
			if (bytes != null && bytes.length >= 4
				...
				try {
					...
					log.warn("decode: caught exception", e);
				} finally {
					try {
						baos.close();
					} catch (Exception e) {
						log.warn("decode: failed to close baos", e);
					}
					try {
						gzis.close();
					} catch (Exception e) {
						log.warn("decode: failed to close gzis", e);
					}
					try {
						bais.close();
					} catch (Exception e) {
						log.warn("decode: failed to close bais", e);
					}
				}
			}
		}
	...
	public static byte getBAD_ENCODING() {
		return BAD_ENCODING;
	}
