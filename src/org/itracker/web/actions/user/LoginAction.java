...
public class LoginAction extends ItrackerBaseAction {
    ...
    public ActionForward execute(ActionMapping mapping, ActionForm form, HttpServletRequest request,
        ...
        try {
            ...
            try {
                ...
                if (authType == AuthenticationConstants.AUTH_TYPE_PASSWORD_PLAIN) {
                    login = (String) request.getAttribute(Constants.AUTH_LOGIN_KEY);
                    String authenticator = (String) request.getAttribute(Constants.AUTH_VALUE_KEY);
                    if (login == null || login.equals("")) {
                        login = (String) PropertyUtils.getSimpleProperty(form, "login");
                    }
                    if (authenticator == null || authenticator.equals("")) {
                        authenticator = (String) PropertyUtils.getSimpleProperty(form, "password");
                    }
                    ...
                    if (authenticator == null || authenticator.equals("")) {
                        authenticator = (String) PropertyUtils.getSimpleProperty(form, "encpassword");
                    }
                ...
                if (redirectIndex > -1) {
                    ...
                    if (extraParamIndex > -1 && lastParamIndex > -1) {
                        redirect = redirect.substring(0, redirectIndex) + "?"
                                + redirect.substring(extraParamIndex + 1, lastParamIndex);
                    } else if (extraParamIndex > -1) {
                        redirect = redirect.substring(0, redirectIndex) + "?" + redirect.substring(extraParamIndex + 1);
                    } else {
                        redirect = redirect.substring(0, redirectIndex);
                    }
                }
                ...
                if (le.getType() == AuthenticatorException.INVALID_PASSWORD) {
                    errors.add(ActionMessages.GLOBAL_MESSAGE, new ActionMessage("itracker.web.error.login.badpass"));
                    ...
                    errors.add(ActionMessages.GLOBAL_MESSAGE, new ActionMessage("itracker.web.error.login.unknown"));
                    ...
                    errors.add(ActionMessages.GLOBAL_MESSAGE, new ActionMessage("itracker.web.error.login.system"));
                	 ...
                	 errors.add(ActionMessages.GLOBAL_MESSAGE, new ActionMessage(le.getMessageKey()));
                } else {
                    errors.add(ActionMessages.GLOBAL_MESSAGE, new ActionMessage("itracker.web.error.login.system"));
                }
        ...
        return (errorMapping == null ? mapping.findForward("login") : errorMapping);
    }

    public User setupSession(String login, HttpServletRequest request, HttpServletResponse response) {
        UserService userService = getITrackerServices().getUserService();
            ...
            if (cookies != null) {
                for (int i = 0; i < cookies.length; i++) {
                    if (Constants.COOKIE_NAME.equals(cookies[i].getName())) {
                        ...
                        if (seperator > 0) {
                            encPassword = cookies[i].getValue().substring(seperator + 1);
                        }
                    }
                }
            }
    ...
    public User setupSession(User user, String encPassword, HttpServletRequest request,
    ...
    private int getConfiguredSessionTimeout() {
        return (getITrackerServices().getConfigurationService().getIntegerProperty("web_session_timeout",
                SESSION_TIMEOUT));
    }
}
