...
public class IssueServiceIT extends AbstractServicesIntegrationTest {
   ...
   public void testCreateIssue() throws Exception {
      ...
      assertSame("new issue id == model issue id", newIssue.getId(), issue.getId());
   ...
   public void testUpdateIssue() throws Exception {
      ...
      int histCount = updateIssue.getHistory().size();
   ...
   public void testUpdateIssueSeverity() throws Exception {
      ...
      int actCount = updateIssue.getActivities().size();
   ...
   public void testMoveIssue() {
      ...
      User user = ((UserService) applicationContext.getBean("userService"))
              .getUser(2);
      int actCount = issue.getActivities().size();
      assertNotNull("user#2", user);
...
//		issue.getFields().add(dateFieldValue);
      ArrayList<IssueField> issueFields = new ArrayList<>(issue.getFields().size() + 1);
   ...
   public void testSetIssueComponents() {
      ...
      assertTrue(updated);
      assertTrue(issueService.getIssueComponentIds(3).contains(1));
   ...
   public void testSetIssueVersions() {
      HashSet<Integer> versionIds = new HashSet<>();
      versionIds.add(1);
      boolean updated = issueService.setIssueVersions(3, versionIds, 2);
      assertTrue(updated);
      assertTrue(issueService.getIssueVersionIds(3).contains(1));
   }
   ...
   public void testGetIssueVersionIds() {
      ...
      assertNotNull(versions);
      assertEquals(1, versions.size());
      assertTrue("version id", versions.contains(1));
   }
   ...
   public void testGetAllIssueAttachmentSize() {
      long size = 0L;
      for (IssueAttachment issueAttachment : issueAttachmentDAO.findAll()) {
         assertNotNull(issueAttachment);
         size += issueAttachment.getSize();
      }
      size = size / 1024;
      assertEquals("total attachmentsSize", (Long) size, issueService.getAllIssueAttachmentSize());
   }
